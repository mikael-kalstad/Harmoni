...
export default class eventDao extends daoParentEvent {
  ...
  getAllEvents(callback) {
    super.query(
      "SELECT event_id, organizer, name, address, capacity, " +
        ...
        "FROM event ORDER BY from_date DESC",
      [],
      callback
    );
  }

  // Get the last 12 uploaded events using Offset to load 12 more if more clicked
  getAllEventsWithOffset(offset: number, callback) {
    super.query(
      "SELECT event_id, organizer, name, address, capacity, " +
        ...
        'DATE_FORMAT(to_date, "%d.%m.%Y %H:%i") as to_date, ' +
        'DATE_FORMAT(from_date, "%d.%m.%Y %H:%i") as from_date ' +
        "FROM event WHERE status = 0 AND event.to_date > NOW() ORDER BY event.from_date ASC LIMIT 12 OFFSET ?",
      [offset],
      callback
    );
  }
  ...
  getCountOfAllEventsNotCancelledNotFinished(callback) {
    super.query(
      "SELECT COUNT(*) as 'count' FROM event WHERE status = 0 AND to_date > NOW()",
      [],
      callback
    );
  }
  ...
  getEventsByCategoryWithOffset(category: string, offset: number, callback) {
    super.query(
      "SELECT event_id, organizer, name, address, capacity, " +
        ...
        'DATE_FORMAT(to_date, "%d.%m.%Y %H:%i") as to_date, ' +
        'DATE_FORMAT(from_date, "%d.%m.%Y %H:%i") as from_date ' +
        "FROM event WHERE category = ? AND status = 0 AND event.to_date > NOW() ORDER BY event.from_date ASC LIMIT 12 OFFSET ?",
      [category, offset],
      callback
    );
  }
  ...
  getCountOfEventsByCategoryNotCancelledNotFinished(
    ...
    super.query(
      "SELECT COUNT(*) as 'count' FROM event WHERE category = ? AND status = 0 AND to_date > NOW()",
      [category],
      callback
    );
  }
