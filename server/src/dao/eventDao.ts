    ...
    getAllEventsWithOffset(offset: number, callback) {
        super.query(
            "SELECT event_id, organizer, name, address, capacity, " +
                ...
                'DATE_FORMAT(to_date, "%d.%m.%Y %H:%i") as to_date, ' +
                'DATE_FORMAT(from_date, "%d.%m.%Y %H:%i") as from_date ' +
                "FROM event WHERE status != 2 AND event.to_date > NOW() ORDER BY event.from_date ASC LIMIT 12 OFFSET ?",
            [offset],
            callback
        );
    }
    ...
    getEventsByCategoryWithOffset(category: string, offset: number, callback) {
        super.query(
            "SELECT event_id, organizer, name, address, capacity, " +
                ...
                'DATE_FORMAT(to_date, "%d.%m.%Y %H:%i") as to_date, ' +
                'DATE_FORMAT(from_date, "%d.%m.%Y %H:%i") as from_date ' +
                "FROM event WHERE category = ? AND status != 2 AND event.to_date > NOW() ORDER BY event.from_date ASC LIMIT 12 OFFSET ?",
            [category, offset],
            callback
        );
    }
